...
Terrain::Terrain(unsigned int xSegs, unsigned int zSegs, std::string fileName) : 
    ...
    // than pixels. 
    //TODO: 
    float scale = 10.0f; // Note that this scales down the values to make
...
void Terrain::Init(){
    ...
    // (Inclass) Build grid of vertices! 
    // looping through all the segments and adding the vertex into a grid
    for(unsigned int z = 0; z < m_zSegments; ++z){
        for(unsigned int x = 0; x < m_xSegments; ++x){
    ...
    // (Inclass) Build triangle strip

    //Todo: simplify 
    for(unsigned int z=0; z < m_zSegments; ++z){
        for(unsigned int x = 0; x < m_xSegments; ++x){
            m_geometry.AddIndex(x+(z*m_zSegments));
            m_geometry.AddIndex(x+(z*m_zSegments)+m_xSegments); //down one row
            m_geometry.AddIndex(x+(z*m_zSegments)+1);           //to the right of the first vertex
